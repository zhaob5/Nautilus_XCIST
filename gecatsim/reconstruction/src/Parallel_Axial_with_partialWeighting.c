...
//	int FOILength,FOIWidth,FOIHeight;
    ...
    BetaS = t->BetaS; // Start Angle of the Short Scan, could smaller than 0
...
//    startangle = t->startangle;
    ...
    w     = (float*)malloc(sizeof(float)*N_2pi);
	 ...
	 for(zi = 0; zi<ZN; zi++)  // For each z location: // create two slices for testing
	 {
         ...
         Beta0 = 0;
         s0 = ceil((Beta0-BetaS) / DeltaFai-0.5); // How many views from start angle to mid plane (rounded to smaller integer)
         s1 = s0-ceil(N_pi*HSCoef);      // start view 
         s2 = s0+ceil(N_pi*HSCoef)-1;    // end view? s2 = ceil((Beta0 + N_Circle * pi + pi ) /DeltaFai);
         ...
         for (int k=0;k<N_2pi;k++)
           { w[k] = 0;} // Create weighting factor for each view
         L = s2-s1+1; // Total numbers of view
         Shift = N_pi - (s0-s1); // = N_pi - N_pi*HSCoef

         if (L<2*delta) // if Total numbers of view is less than 120, delta was hard coded as 60?
         {
             for (int k=0;k<L;k++)
                 ...
                 for(xi=0;xi<XN;xi++)
                 {
                    ...
                    if ((U>0)&&(U<YL)&&(V>0)&&(V<ZL))
                    {
                          ...
                          weight2 = w[d2]; // conj
                           ...
                           t->RecIm[yi][xi][zi]=t->RecIm[yi][xi][zi]+weight*touying*DeltaFai; // Eq.9 in Tang_2006 paper
                           //t->RecIm[yi][xi][zi]=t->RecIm[yi][xi][zi]+w[d1]*touying*DeltaFai;
