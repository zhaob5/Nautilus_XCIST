...
def halfscan_helical_equiAngle(cfg, prep):
    print("Prep range:", np.min(prep), np.max(prep))
    ...
    # Suppose you want half-scan from 0° to 200°
    start_angle = 0
    end_angle   = 200
    
    # Convert to indices
    start_idx = int(start_angle / 360 * ViewN)
    end_idx   = int(end_angle   / 360 * ViewN)
    
    # Slice projections
    subset_prep = np.zeros_like(prep)
    subset_prep[:,:,start_idx:end_idx] = prep[:,:,start_idx:end_idx]
    prep = subset_prep
    # Keep ViewN the same as before

    ViewN = end_idx - start_idx
    
    # Update Beta range
    BetaS = math.radians(start_angle)
    BetaE = math.radians(end_angle)
    print("BetaS (deg):", BetaS*180/np.pi)
    print("BetaE (deg):", BetaE*180/np.pi)
