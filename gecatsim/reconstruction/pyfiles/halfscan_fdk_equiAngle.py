...
def halfscan_fdk_equiAngle(cfg, prep):
    ...
    #Dg=Dgy
    Dg = np.append(Dgy, np.zeros((YL, ZL, ProjScale - cfg.protocol.viewCount)), axis=2) # try padding (cut unwantted section from the scanned data)
    ...
    for ProjIndex in range(0, ProjScale): # ProjScale = viewsPerRotation
        for j in range(ZL):
            TempData = np.ones(YL)
            for k in range(YL):
                TempData[k] = Dg[k, j, ProjIndex] # Dg = data based on viewCount
