...

{
    ...
    */
    ...
    // For multi-threaded CPU
    int viewSize = (nrdetcols + 2) * (nrdetrows + 2);
    memset(view, 0, viewSize * sizeof(float));
    
    #pragma omp parallel
    {
        float* view_local = (float*)calloc(viewSize, sizeof(float));
        /*#pragma omp single
        {
            printf("DD3 row parallel threads = %d\n", omp_get_num_threads());
        }*/
        #pragma omp for schedule(static)
        for (int rownr = 0; rownr < nrrows; rownr++)
        {
            float *viewCopy_local;
            void *pImg_local;
            byte *p_mask_local;
    
            // recompute row-local variables here
            // same as old code, but use local pointers
    
            if (*(detX+2) > *(detX+1))
            {
                viewCopy_local = view_local;
            }
            else
            {
                viewCopy_local = view_local + (nrdetcols+1)*(nrdetrows+2);
            }
    
            // compute pImg_local for this row from pOrig
            // compute p_mask_local for this row
            // compute imgX, imgXstep, imgZ, imgZstep
            if (useUInt16) {
                pImg_local = static_cast<unsigned short*>(pOrig) + rownr * num_trans_plane_step;
            } else {
                pImg_local = static_cast<float*>(pOrig) + rownr * num_trans_plane_step;
            }
            
            p_mask_local = p_mask + rownr * nrcols;
            
            float mag_fac = y0 / (y0 - ((nrrows - 1) * 0.5f - rownr) * vox_xy_size);
            float imgXstep_local = mag_fac * vox_xy_size;
            float imgX_local = -x0 / (y0 - ((nrrows - 1) * 0.5f - rownr) * vox_xy_size)
                               * (((nrrows - 1) * 0.5f - rownr) * vox_xy_size)
                               - (nrcols * 0.5f * vox_xy_size) * mag_fac;
            
            float imgZstep_local = mag_fac * vox_z_size;
            float imgZ_local = z0 - (nrplanes * 0.5f * vox_z_size + z0) * mag_fac;

            DD3ProjRow_roi_notrans_mm(
                imgX_local, imgXstep_local, nrcols,
                imgZ_local, imgZstep_local, nrplanes,
                pImg_local, detXcopy, increment, detZ, scalesCopy,
                z0, viewCopy_local, nrdetrows, nrdetcols,
                num_trans_rowstep, p_mask_local
            );
        }
