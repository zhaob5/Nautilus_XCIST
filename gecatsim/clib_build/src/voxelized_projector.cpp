...
void voxelized_projector(
  ...
  {
  //Report(sprintf(OutputString, "omp_get_max_threads() = %d\n", omp_get_max_threads()));
  float *xds = NULL, *yds = NULL, *zds = NULL, x0 = 0, y0 = 0, z0 = 0, totalWt = 0, viewangle;
  //float *xds = NULL, *yds = NULL, *zds = NULL;
  ...
  *Status = 0; 
  ...
  /*
  //for CPU speed up:
    int projSize = nrdetcols * nrdetrows;
    int m = MaterialIndexInMemory - 1;
    
    #pragma omp parallel
    {
        
        float *localViewProj = new float[projSize];
        float *subViewProj   = new float[projSize];
    
        memset(localViewProj, 0, projSize * sizeof(float));
        //Report(sprintf(OutputString, "nSubSources = %d\n", nSubSources));
        #pragma omp for
        for (int i = 0; i < nSubSources; i++) {
            float x0 = sourcePoints[3*i + 0];
            float y0 = sourcePoints[3*i + 1];
            float z0 = sourcePoints[3*i + 2];
            float wt = modules.sourceWeights[i];
    
            memset(subViewProj, 0, projSize * sizeof(float));
    
            float viewangle = 0.0f;
            float zoffset = 0.0f;
    
            DD3Proj_roi_notrans_mm(
                x0, y0, z0,
                nrdetcols, nrdetrows,
                xds, yds, zds,
                phantom.xoff[m], phantom.yoff[m], phantom.zoff[m],
                &viewangle, &zoffset, 1,
                subViewProj,
                phantom.dims[m][0], phantom.dims[m][1], phantom.dims[m][2],
                (phantom.volume)[m],
                phantom.dxy[m], phantom.dz[m],
                phantom.xy_mask[m]
            );
    
            for (int detIndex = 0; detIndex < projSize; detIndex++) {
                localViewProj[detIndex] += wt * subViewProj[detIndex];
            }
        }
        ...
        {
            for (int detIndex = 0; detIndex < projSize; detIndex++) {
                thisViewProj[detIndex] += localViewProj[detIndex];
            }
        }
    
        delete[] localViewProj;
        delete[] subViewProj;
    }*/
  //end CPU speed up
